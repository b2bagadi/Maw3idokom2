...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
    try {
        ...
        // ----- Load business (tenant) for notification -----
        ...
        const notificationData = {
            ...
            date: newStart.toLocaleDateString(locale),
            time: newStart.toLocaleTimeString(locale, { hour: '2-digit', minute: '2-digit' }),
        };

        // ----- Send notifications (client‑side will also redirect) -----
        let whatsappLink = '';
