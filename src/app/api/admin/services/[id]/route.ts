...
export async function DELETE(
  ...
  try {
    ...
    const { appointments, staffServices } = await import('@/db/schema');

    // 1. Delete appointments for this service
    await db.delete(appointments).where(eq(appointments.serviceId, serviceId));

    // 2. Delete staff-service associations
