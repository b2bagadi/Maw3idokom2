...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!superAdminId) {
      console.error('[Admin Appointments API] Unauthorized access attempt');
    ...
    console.log('[Admin Appointments API] Request received from super admin:', superAdminId);
    ...
    console.log('[Admin Appointments API] Query params:', {
      tenantIdParam, statusParam, search, fromParam, toParam, staffIdParam, page, limit
    });
    ...
    if (statusParam) {
      // Handle multiple status values (comma-separated or array format)
    ...
    if (fromParam) {
      conditions.push(sql`${appointments.startTime} >= ${fromParam}`);
    }

    if (toParam) {
      conditions.push(sql`${appointments.startTime} <= ${toParam}`);
    }
    ...
    console.log('[Admin Appointments API] Total appointments found:', total);
