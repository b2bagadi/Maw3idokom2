...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
    try {
        ...
        await db.update(appointments).set({ status: 'CANCELLED', updatedAt: new Date().toISOString() }).where(eq(appointments.id, appointmentId));
        ...
        const notificationData = {
            customerName: appt.guestName || 'Guest',
        ...
        return NextResponse.json({ success: true, message: 'Appointment cancelled', whatsappLink: whatsappLink || undefined });
    } catch (error) {
