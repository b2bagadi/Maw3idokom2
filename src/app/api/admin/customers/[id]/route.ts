...
import { db } from '@/db';
import { users, appointments, tenants, services, staff, superAdmins, reviews, staffServices, workingHours } from '@/db/schema';
...
async function verifySuperAdmin(request: NextRequest): Promise<{ superAdminId: number; username: string } | null> {
  try {
    ...
    return { superAdminId: decoded.id, username: decoded.username };
  } catch (error) {
...
export async function DELETE(
  ...
  try {
    ...
    // Get all appointments for this customer to clean up their related data (reviews) first
    const customerAppointments = await db.select({ id: appointments.id })
      .from(appointments)
      .where(eq(appointments.customerId, customerId));

    // Delete reviews associated with these appointments
    for (const appt of customerAppointments) {
      await db.delete(reviews).where(eq(reviews.appointmentId, appt.id));
    }

    // Delete the appointments permanently
    ...
    // Check if user has a staff entry and clean up related data
    const staffEntry = await db.select().from(staff).where(eq(staff.userId, customerId)).limit(1);

    if (staffEntry.length > 0) {
      const staffId = staffEntry[0].id;
      ...
      // Nullify staffId in appointments
