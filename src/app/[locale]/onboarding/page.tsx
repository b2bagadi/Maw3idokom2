...
export default function BusinessDashboardPage() {
  ...
  const handleConfirmAction = async () => {
    ...
    try {
      const token = localStorage.getItem('business_token');
      const response = await fetch(`/api/business/appointments/${selectedAppointment.id}/confirm?id=${selectedAppointment.id}`, {
        method: 'PUT',
        headers: {
  ...
  const handleRejectAction = async () => {
    ...
    try {
      const token = localStorage.getItem('business_token');
      const response = await fetch(`/api/business/appointments/${selectedAppointment.id}/reject?id=${selectedAppointment.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  const handleRescheduleAction = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/business/appointments/${selectedAppointment.id}/reschedule?id=${selectedAppointment.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
