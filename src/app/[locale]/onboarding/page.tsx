...
import { ScheduleTab } from '@/components/business/ScheduleTab';

type Section = 'appointments' | 'profile' | 'services' | 'staff' | 'gallery' | 'reviews' | 'schedule';
...
export default function BusinessDashboardPage() {
  ...
  const handleCategoryChange = (val: string) => {
    ...
    if (cat) {
      setProfile({
        ...profile,
        categoryId: val ? parseInt(val) : null,
        businessType: cat.nameEn
      });
    }
  };
  ...
  /* Smart Coordinate Handler (Paste "Lat, Lng") */
    ...
    if (value.includes(',')) {
      ...
      if (parts.length === 2) {
        ...
        if (!isNaN(lat) && !isNaN(lng)) {
          ...
          toast.success('Coordinates auto-updated from paste!');
          return;
        }
      }
    }
    // Normal single value update (allow string for typing)
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      // Ensure coords are sent as numbers or null
      if (payload.latitude !== undefined && payload.latitude !== null && payload.latitude !== '') {
        payload.latitude = parseFloat(String(payload.latitude));
      } else {
        payload.latitude = null;
      }

      if (payload.longitude !== undefined && payload.longitude !== null && payload.longitude !== '') {
        payload.longitude = parseFloat(String(payload.longitude));
      } else {
      ...
      const response = await fetch('/api/business/profile', {
        ...
        body: JSON.stringify({ ...payload, galleryImages: JSON.stringify(galleryUrls) })
      });

      if (response.ok) {
        toast.success('✅ ' + t('profileUpdated'));
        // Reload data to ensure state matches server
        await loadData();
      } else {
        const data = await response.json();
      ...
      <CardContent className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            <Label>{t('type')}</Label>
            <Select
              value={profile.categoryId ? String(profile.categoryId) : ''}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
          <div>
            ...
            <Input
              ...
              placeholder="34.0522 or paste lat,lng"
            />
          </div>
          <div>
            <Label className="flex items-center gap-2">
