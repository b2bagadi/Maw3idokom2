...
export default function AdminDashboardPage() {
  ...
  const handleConfirmAction = async () => {
    ...
    try {
      const token = localStorage.getItem('admin_token');
      const res = await fetch(`/api/admin/appointments/${selectedAppointment.id}/confirm`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
        },
        body: JSON.stringify({ notifyVia: notificationMethod })
      });
  ...
  const handleRescheduleAction = async () => {
    ...
    try {
      const token = localStorage.getItem('admin_token');
      const res = await fetch(`/api/admin/appointments/${selectedAppointment.id}/reschedule`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
