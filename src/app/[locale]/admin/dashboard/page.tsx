  ...
  const handleConfirmAction = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/admin/appointments/${selectedAppointment.id}/confirm`, {
        ...
        body: JSON.stringify({ notificationMethod: notificationMethod === 'whatsapp' ? 'none' : 'email' })
      });
      ...
      if (response.ok) {
        ...
        if (notificationMethod === 'whatsapp') {
          ...
          const service = selectedAppointment.service?.nameEn || 'service';
          const date = new Date(selectedAppointment.startTime).toLocaleDateString();
          const message = encodeURIComponent(
            `Hello ${name}, your appointment for ${service} on ${date} is confirmed. We look forward to seeing you!`
          );
    ...
    try {
      ...
      const response = await fetch(`/api/admin/appointments/${selectedAppointment.id}/cancel`, {
        ...
        body: JSON.stringify({
          ...
          notificationMethod: notificationMethod === 'whatsapp' ? 'none' : 'email'
        })
      });
      ...
      if (response.ok) {
        ...
        if (notificationMethod === 'whatsapp') {
          ...
          const name = selectedAppointment.guestName || 'Guest';
          const message = encodeURIComponent(
            `Hello ${name}, we regret to inform you that your appointment has been rejected. Reason: ${cancellationReason}`
          );
          window.open(`https://wa.me/${phone}?text=${message}`, '_blank');
        }
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (notificationMethod === 'whatsapp') {
          const phone = selectedAppointment.guestPhone;
          const name = selectedAppointment.guestName || 'Guest';
          const newDateStr = dateTime.toLocaleString();
          const message = encodeURIComponent(
            `Hello ${name}, your appointment has been rescheduled to ${newDateStr}. Reason: ${rescheduleReason || 'Schedule adjustment'}`
          );
          window.open(`https://wa.me/${phone}?text=${message}`, '_blank');
        }
                      ...
                      <div className="flex justify-between items-start flex-wrap gap-4">
                        <div className="flex-1 min-w-[200px]">
                          ...
                          <div className="mt-2 text-sm text-muted-foreground space-y-1">
                            {appt.guestEmail && <div>✉️ {appt.guestEmail}</div>}
                            {appt.guestPhone && <div>📱 {appt.guestPhone}</div>}
                          ...
                          <div className="flex flex-wrap gap-2 mt-4">
                            ...
                            {appt.guestPhone && (
                              <Button
                                ...
                                onClick={() => {
                                  const phone = appt.guestPhone.replace(/[^\d]/g, '');
                            ...
                            {appt.guestEmail && (
                              <Button
                                ...
                                onClick={() => window.location.href = `mailto:${appt.guestEmail}`}
                              >
