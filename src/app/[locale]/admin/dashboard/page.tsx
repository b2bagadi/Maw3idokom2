...
type Section = 'businesses' | 'services' | 'staff' | 'appointments' | 'contacts' | 'i18n' | 'settings' | 'password' | 'customers' | 'reviews' | 'categories';
...
export default function AdminDashboardPage() {
      ...
      switch (activeSection) {
        case 'businesses':
          ...
          if (bizRes.ok) {
            ...
            // Also load categories for the selector
  ...
  const handleToggleActivation = async (business: any, currentStatus: boolean) => {
    ...
    try {
      ...
      if (response.ok) {
        toast.success(data.message || `Business ${!currentStatus ? 'activated' : 'deactivated'} successfully`);
      } else {
        // Revert on failure
  ...
  const handleToggleCustomerActivation = async (customer: any, currentStatus: boolean) => {
    // Optimistic update
    const previousCustomers = [...customers];
    setCustomers(customers.map(c =>
      c.id === customer.id ? { ...c, isActive: !currentStatus } : c
    ));

    try {
      const token = localStorage.getItem('admin_token');
      const response = await fetch(`/api/admin/customers/${customer.id}/activate`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
      ...
      if (response.ok) {
        toast.success(data.message || `Customer ${!currentStatus ? 'activated' : 'deactivated'} successfully`);
      } else {
        // Revert on failure
    ...
    try {
      ...
      switch (activeSection) {
        ...
        case 'i18n':
          ...
          break;
        case 'contacts':
          endpoint = `/api/admin/contacts?id=${selectedItem.id}`;
          break;
        case 'reviews':
          endpoint = `/api/reviews/${selectedItem.id}`;
          break;
        case 'categories':
          endpoint = `/api/admin/categories/${selectedItem.id}`;
          break;
        case 'customers':
          endpoint = `/api/admin/customers/${selectedItem.id}`;
          break;
        case 'appointments':
          endpoint = `/api/admin/appointments/${selectedItem.id}`;
          break;
      }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      switch (activeSection) {
        ...
        case 'categories':
          endpoint = selectedItem
            ? `/api/admin/categories/${selectedItem.id}`
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="md:col-span-2">
            <Label>Password {selectedItem && '(leave blank to keep current password)'}</Label>
            <Input
              type="password"
              value={formData.password || ''}
              onChange={(e) => setFormData({ ...formData, password: e.target.value })}
              placeholder={selectedItem ? "New password (optional)" : "Business owner password"}
              autoComplete="off"
            />
          </div>
        </div>
      ...
      <div className="space-y-6">
        ...
        <div>
          ...
          <Input
            ...
            placeholder="Scissors or https://example.com/logo.png"
          />
          <p className="text-xs text-muted-foreground mt-1">
            Enter a Lucide icon name (Scissors, Car, Dumbbell) or an image URL
          </p>
        </div>
        ...
        >
          <Save className="w-4 h-4 mr-2" />
          {isLoading ? 'Saving...' : selectedItem ? 'Update Category' : 'Create Category'}
        </Button>
      </div>
    );
  };
  ...
  const renderContent = () => {
    ...
    switch (activeSection) {
                    ...
                    <CardHeader>
                      <div className="flex justify-between items-start">
                        ...
                        <div className="flex items-center gap-2 border-l pl-4">
                          <Label htmlFor={`active-customer-${customer.id}`} className="text-xs cursor-pointer">
                            {customer.isActive ? 'Active' : 'Inactive'}
                          </Label>
                          <Switch
                            id={`active-customer-${customer.id}`}
                            checked={customer.isActive}
                            onCheckedChange={() => handleToggleCustomerActivation(customer, customer.isActive)}
                          />
                          <Button size="sm" variant="destructive" className="ml-2" onClick={() => handleDelete(customer)}>
                            <Trash2 className="w-4 h-4" />
                          </Button>
                        </div>
                      </div>
      ...
      case 'appointments':
        return <AppointmentsView onAction={handleAppointmentAction} onDelete={(appointment: any) => {
                      ...
                      <div className="flex justify-between items-start gap-4">
                        <div className="flex-1">
                          ...
                          <p className="text-xs text-muted-foreground">
                            <strong>{review.businessName}</strong> • {review.customerName} •
                          ...
                          {review.businessReply && (
                            <div className="mt-3 p-3 bg-blue-50 rounded-lg border-l-4 border-blue-400">
                              ...
                              <p className="text-xs text-blue-600 mt-1">
                                {new Date(review.businessRepliedAt).toLocaleDateString()}
                              </p>
                            </div>
                          )}
                        </div>
                        <div className="flex gap-2 flex-shrink-0">
                          <Button
                            ...
                            onClick={async () => {
                              ...
                              if (res.ok) {
                                toast.success(review.isApproved ? 'Review disapproved' : 'Review approved');
                                loadData();
                              } else {
        ...
        return (
          <div>
            <div className="mb-6">
              ...
              <p className="text-muted-foreground">Manage customer reviews and business replies</p>
            </div>
            <ReviewsManagement reviews={reviews} onUpdate={loadData} userRole="admin" />
          </div>
        );
    ...
    <div className="flex min-h-screen bg-gradient-to-br from-background to-muted/20">
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          {activeSection === 'categories' && renderCategoryForm()}
