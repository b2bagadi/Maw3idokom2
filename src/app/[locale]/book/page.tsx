...
import { Calendar as CalendarIcon, Clock, DollarSign, User, ArrowLeft, Loader2, MapPin } from 'lucide-react';
...
function BookingContent() {
    ...
    const loadBusinessData = async (slug: string) => {
        try {
            setIsLoading(true);
            const businessRes = await fetch(`/api/public/tenants?slug=${slug}`);
            if (businessRes.ok) {
                const businessData = await businessRes.json();
                if (businessData.tenants && businessData.tenants.length > 0) {
                    const tenant = businessData.tenants[0];
                    setBusiness(tenant);
                ...
                <main className="container mx-auto px-4 py-12">
                    <div className="max-w-6xl mx-auto">
                        <div className="text-center mb-12">
                            <h1 className="text-4xl font-bold mb-4">Select a Business</h1>
                            <p className="text-muted-foreground text-lg">Choose a business to book your appointment</p>
                        </div>
                        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {businesses.map((biz) => (
                                <Card key={biz.id} className="hover:shadow-lg transition-all duration-300 cursor-pointer" onClick={() => handleBusinessSelect(biz.slug)}>
                                    <CardHeader>
                                        <CardTitle>{biz.nameEn}</CardTitle>
                                        <CardDescription>{biz.descriptionEn}</CardDescription>
                                    </CardHeader>
                                    <CardContent>
            ...
            <header className="border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
                <div className="container mx-auto px-4 py-4 flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        <Link href="/book" className="flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground">
                            <ArrowLeft className="h-4 w-4" />
                            All Businesses
                        </Link>
                        <span className="font-semibold border-l pl-4">{business?.nameEn || business?.name}</span>
                    </div>
                    <LanguageSwitcher />
                </div>
            </header>
                                        ...
                                        <div className="grid md:grid-cols-2 gap-4">
                                            {services.map((service) => (
                                                <Card
                                                    ...
                                                    onClick={() => { setSelectedService(service.id); if (step === 1) setStep(2); }}
                                                >
                                                    <CardHeader>
                                ...
                                <Card>
                                    ...
                                    <CardContent>
                                        <RadioGroup
                                            ...
                                            onValueChange={(value) => { setSelectedStaff(parseInt(value)); if (step === 2) setStep(3); }}
                                        >
                                    ...
                                    <CardContent className="space-y-6">
                                        <div>
                                            ...
                                            <Calendar
                                                ...
                                                onSelect={(date) => { setSelectedDate(date); if (step === 3) setStep(4); }}
