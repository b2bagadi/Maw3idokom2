...
export default function BusinessSignupPage() {
  ...
  // Use useLocale to determine language for category names
  const locale = usePathname().split('/')[1] || 'en'; // Simple extraction or use hook
  ...
  const handleCategoryChange = (val: string) => {
    // val is likely the ID as string. Find the category to set businessType text as fallback
    const cat = categories.find(c => String(c.id) === val);
    if (cat) {
      setFormData({
        ...formData,
        categoryId: val,
        businessType: cat.nameEn // Fallback text
      });
    }
  };
