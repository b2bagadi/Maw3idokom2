    ...
    const fetchAppointments = async () => {
        ...
        try {
            ...
            const endpoint = scope === 'admin'
                ? `/api/admin/businesses/${tenantId}/appointments` // Admin specific
                : `/api/appointments`; // Business/Client specific (though business usually implies 'me')

            // For this implementation, we might use a unified endpoint or handle the difference
            // If scope is business, we assume the API handles 'me' via token.
            // If scope is admin, we need the specific tenant endpoint.

            // However, the requirement says:
            // Admin: /api/admin/businesses/[id]/appointments
            // Business: /api/appointments?tenantId=me (or just /api/appointments)
